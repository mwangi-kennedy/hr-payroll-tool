const db = require('../db');
const { daysBetween, getNoticeDays, MIN_NOTICE_DAYS, TEAM_COVERAGE_THRESHOLD, ESCALATION_DAYS } = require('../services/leaveRules');

function createLeaveRequest(req, res) {
    const { employee_id, start_date, end_date, leave_type, reason } = req.body;

    if (!employee_id || !start_date || !end_date || !leave_type) {
        return res.status(400).json({ error: 'Missing required fields' });
    }

    const employee = db.prepare('SELECT * FROM employees WHERE id = ? AND is_active = 1').get(employee_id);
    if (!employee) return res.status(404).json({ error: 'Employee not found or inactive' });

    if (new Date(end_date) < new Date(start_date)) {
        return res.status(400).json({ error: 'end_date cannot be before start_date' });
    }

    const today = new Date().toISOString().split('T')[0];
    if (new Date(start_date) < new Date(today)) {
        return res.status(400).json({ error: 'Cannot request leave for a date in the past' });
    }

    const days_requested = daysBetween(start_date, end_date);
    const notice_days = getNoticeDays(today, start_date);
    const low_notice = notice_days < MIN_NOTICE_DAYS;

    const result = db.prepare(`
        INSERT INTO leave_requests (employee_id, start_date, end_date, days_requested, leave_type, reason)
        VALUES (?, ?, ?, ?, ?, ?)
    `).run(employee_id, start_date, end_date, days_requested, leave_type, reason || null);

    const newRequest = db.prepare('SELECT * FROM leave_requests WHERE id = ?').get(result.lastInsertRowid);
    res.status(201).json({ ...newRequest, low_notice, notice_days });
}

function listLeaveRequests(req, res) {
    const { status } = req.query;
    const requests = status
        ? db.prepare('SELECT * FROM leave_requests WHERE status = ?').all(status)
        : db.prepare('SELECT * FROM leave_requests').all();

    const now = new Date();
    const enriched = requests.map(r => {
        const pendingDays = Math.floor((now - new Date(r.requested_at)) / (1000 * 60 * 60 * 24));
        return { ...r, escalated: r.status === 'pending' && pendingDays >= ESCALATION_DAYS };
    });

    res.json(enriched);
}

function decideLeaveRequest(req, res) {
    const { id } = req.params;
    const { decision, decided_by } = req.body;

    if (!['approved', 'rejected'].includes(decision)) {
        return res.status(400).json({ error: "decision must be 'approved' or 'rejected'" });
    }

    const leaveRequest = db.prepare('SELECT * FROM leave_requests WHERE id = ?').get(id);
    if (!leaveRequest) return res.status(404).json({ error: 'Leave request not found' });
    if (leaveRequest.status !== 'pending') {
        return res.status(400).json({ error: `Request already ${leaveRequest.status}` });
    }

    if (decision === 'approved') {
        const employee = db.prepare('SELECT * FROM employees WHERE id = ?').get(leaveRequest.employee_id);

        if (employee.team_id) {
            const teamSize = db.prepare('SELECT COUNT(*) as count FROM employees WHERE team_id = ? AND is_active = 1').get(employee.team_id).count;

            const overlapping = db.prepare(`
                SELECT lr.* FROM leave_requests lr
                JOIN employees e ON e.id = lr.employee_id
                WHERE e.team_id = ? AND lr.status = 'approved'
                AND lr.start_date <= ? AND lr.end_date >= ?
            `).all(employee.team_id, leaveRequest.end_date, leaveRequest.start_date);

            const coverageRatio = (overlapping.length + 1) / teamSize;
            if (coverageRatio > TEAM_COVERAGE_THRESHOLD) {
                return res.status(409).json({
                    error: 'Approving this would leave the team under-covered',
                    team_size: teamSize,
                    already_approved_overlapping: overlapping.length,
                    coverage_ratio: coverageRatio
                });
            }
        }
    }

    db.prepare(`
        UPDATE leave_requests SET status = ?, decided_at = CURRENT_TIMESTAMP, decided_by = ?
        WHERE id = ?
    `).run(decision, decided_by || null, id);

    res.json(db.prepare('SELECT * FROM leave_requests WHERE id = ?').get(id));
}

module.exports = { createLeaveRequest, listLeaveRequests, decideLeaveRequest };